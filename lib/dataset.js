...
_.extend(CDCDataset.prototype, {
  ...
  fromURL: function(url, options) {
    ...
    if (options.archiveDir) {
      writer = fs.createWriteStream(path.join(archiveDir, 'cdc.txt'));
      req.pipe(writer);
    }
...
_.extend(GSCQuicksearchDataset.prototype, {
  ...
  fromFile: function(fileName, options) {
    options = options || {};
    var dataset = this;
    //return this.bindStreamEvents(fs.createReadStream(fileName), options);
    fs.readFile(fileName, function(err, data) {
      var clean = data.toString();
      clean = clean.replace(/^\)\]\}',/, '');
      clean = clean.replace(/[\\b]([A-Za-z])/, "\\\\$1");
      var j = JSON.parse(clean);
      _.each(j.MATCHES.DATA, function(data) {
        dataset.emit('data', dataset.createRecord(data));
      });
      dataset.emit('end');
    });
